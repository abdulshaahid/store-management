...
export default function Home() {
  ...
  const confirmAddToCart = () => {
    ...
    const quantity = Number.parseInt(quantityInput) || 1
                  ...
                  >
                    <div className="p-3 sm:p-4">
                      ...
                      <div className="flex justify-between items-center">
                        <span className="font-bold text-base sm:text-lg"><span className="text-black/50 text-xs" >AED.</span>{product.price.toFixed(2)}</span>
                        <button
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm animate-in fade-in zoom-in-95">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg sm:text-xl font-bold">{selectedProduct.name}</h3>
              <button onClick={() => setShowQuantityModal(false)} className="text-gray-500 hover:text-gray-700">
                <X className="w-6 h-6" />
              </button>
            </div>
            <p className="text-gray-600 mb-2 text-sm sm:text-base">{selectedProduct.description}</p>
            <p className="text-2xl sm:text-3xl font-bold mb-6">${selectedProduct.price.toFixed(2)}</p>

            <div className="mb-6">
              <label className="block text-sm font-semibold mb-2">Quantity ({selectedProduct.unit})</label>
              <div className="flex items-center gap-3">
                <button
                  onClick={() => setQuantityInput(Math.max(1, Number.parseInt(quantityInput) - 1).toString())}
                ...
                <input
                  type="number"
                  value={quantityInput}
                  onChange={(e) => setQuantityInput(e.target.value || "1")}
                  className="flex-1 border border-gray-300 rounded-lg px-4 py-2 text-center font-semibold text-base"
                  min="1"
                />
                <button
                  onClick={() => setQuantityInput((Number.parseInt(quantityInput) + 1).toString())}
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 border-2 border-gray-300 text-black py-3 rounded-lg font-semibold hover:bg-gray-50 transition text-sm sm:text-base"
              >
                Cancel
              </button>
              <button
                onClick={confirmAddToCart}
                className="flex-1 bg-lime-300 hover:bg-lime-400 text-black py-3 rounded-lg font-semibold transition text-sm sm:text-base"
              >
                Add to Cart
              </button>
            </div>
    ...
    <div className="min-h-screen bg-white pb-32 md:pb-24">
      ...
      <main className="px-4 sm:px-6 lg:px-8 py-8 max-w-7xl mx-auto">
        ...
        <div className="bg-gradient-to-br from-lime-300 to-lime-200 rounded-2xl p-6 sm:p-8 text-black mb-8 shadow-lg">
          ...
          <h2 className="text-4xl sm:text-5xl font-bold"><span className="text-black/70 text-lg">AED.</span>{total.toFixed(2)}</h2>
        </div>
        ...
        {cart.length > 0 ? (
          <div className="space-y-4 mb-8">
            ...
            {cart.map((item) => (
              <div key={item.id} className="flex gap-4 bg-gray-50 p-4 rounded-xl border border-gray-200">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex justify-between items-center">
                    <span className="font-bold text-sm sm:text-base"><span className="text-black/50 text-xs">AED.</span>{item.price.toFixed(2)}</span>
        ...
        {cart.length > 0 && (
          <div className="bg-gray-50 border border-gray-200 rounded-xl p-6 space-y-3">
            <div className="flex justify-between text-base">
              <span className="text-gray-600">Sub total :</span>
              <span className="font-semibold"><span className="text-black/50 text-xs">AED.</span>{subtotal.toFixed(2)}</span>
            </div>
            <div className="flex justify-between text-base items-center">
              ...
              {editingDiscount ? (
                ...
                >
                  <span className="text-black/50 text-xs">AED.</span>{discount.toFixed(2)}
                </button>
              )}
            </div>
            <div className="flex justify-between text-lg font-bold border-t border-gray-300 pt-3">
              <span>Total :</span>
              <span><span className="text-black/50 text-xs">AED.</span>{total.toFixed(2)}</span>
            </div>
          </div>
        )}
      </main>
